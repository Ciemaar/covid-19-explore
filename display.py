...
def column_summary(inframe,column):
    ret = []
    deltas, smoothed_deltas, ddeltas, smoothed_ddeltas, dddeltas, smoothed_dddeltas,predicted_deltas = calculate_discrete_derivatives(inframe, column)
...
def state_summary(full_df, lookback, label=None):
    ...
    diag_to_death = pn.widgets.IntSlider(name='Days from diagnosis to death to estimate CFR', value=7, start=1, end=lookback)
